          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="form-group">
                ...
                <input type="text" id="tag" placeholder="Tag here" name="tag" class="form-control">
              </div>
            </div>
          </div>
        </div>
...
$.validator.setDefaults({
    submitHandler: function () {
      // $('#quickForm').submit();    
      $.post($('#quickForm').attr('action'), {'id':$('#tag-id').val(), "_token": "{{ csrf_token() }}", 'tag':$('#tag').val()}, function(resp){
        console.log(resp);
        if(resp.status=='ok'){
